...
public class StudentsList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog.show();
        StringRequest stringRequest = new StringRequest(Request.Method.POST, URLs.URL_STD_LIST, new Response.Listener<String>() {
            @Override
                ...
                try {
                    ...
                    for (int i = 0; i < tecs.length(); i++) {
                        ...
                        if (!name.equals("")) {
                            ...
                            // To Dismiss progress dialog
                            progressDialog.dismiss();
                            Toast.makeText(StudentsList.this, response, Toast.LENGTH_LONG).show();
                        }
                        DbListData tecData = new DbListData(name, stdUniqueUser, email, age, Nmbr, gender, image);
                        data.add(tecData);

                    }
                    adaptor = new DbListDataAdopter(StudentsList.this, data, "StudentsList");
                    recyclerView.setAdapter(adaptor);


                } catch (JSONException e) {
                    progressDialog.dismiss();
                    Toast.makeText(StudentsList.this, response, Toast.LENGTH_SHORT).show();
                    e.printStackTrace();
                }
            }
            ...
            public void onErrorResponse(VolleyError error) {
                progressDialog.dismiss();
                Toast.makeText(StudentsList.this, "Internet Slow/Not Connected", Toast.LENGTH_SHORT).show();
            }
        }) {
            @Override
    ...
    public void headerClick(View view) {
        ...
        switch (id) {
            ...
            case R.id.homeBtn:
               finish();
                Intent hm = new Intent(StudentsList.this, TeacherHome.class);
                startActivity(hm);
                break;
            case R.id.logoutBtn:
                sp.clearPref();
